...
const files = (await readdir("./public/documents")).filter(
  (i) => i !== "Ecuador v Mexico.pdf" && i.endsWith(".pdf")
);
        ...
        messages: [
          {
            ...
            content: [
              {
                type: "text",
                text: "This case is being considered in from of the International Court of Justice at Model UN. Respond with regard to this case.",
              },
              {
                type: "file",
...
const results = await processInChunks(files, 10);
